...
- デバッグ用 SD では `tools/inject-sd.sh --uart-log` で `dtoverlay=disable-bt`、`enable_uart=1`、`init_uart_baud=115200` を設定し、GPIO14/15 の PL011 UART から session 確立前の進行状況を読めるようにする。
